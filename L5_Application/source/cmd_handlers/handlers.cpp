...
CMD_HANDLER_FUNC(orientationCmd)
{
 ...
 CMD_HANDLER_FUNC(statusHandler) {
	...
	/*	need to take into account of consumer*/
	if (cmdParams == "resume producer") {
		 producer->resume(); //vTaskSuspend(producer->getTaskHandle());
	} else if (cmdParams == "suspend producer") {
		 producer->suspend();//vTaskResume(producer->getTaskHandle());
