...
/*
 * Jonathan Chen and Andrew Javier
 */
...
I2C_Base::mStateMachineStatus_t I2C_Base::i2cStateMachine()
{
    enum {
		...
		slaveReadAckSent 	  = 0xA8,		//address and read received
        dataAckedByMaster	  = 0xB8,		//data transmit
        lastDataAckedByMaster = 0xC8,		//
        dataNackedByMaster	  = 0xC0,

		//Slave Receiver States:
		selfSlaveAddressAck				= 0x60,
		generalCallAcked 				= 0x70,
		prevDataReceivedAckedbySlave	= 0x80,
		prevDataReceivedNackedbySlave   = 0x88,
		stopReceieved 					= 0xA0,
    ...
    #define clearSIFlag()       mpI2CRegs->I2CONCLR = (1<<3)		//I2C control clear register, 1 = cleared, 0 = no
    #define setSTARTFlag()      mpI2CRegs->I2CONSET = (1<<5)		//I2C Control set register, 1 = set
    ...
    switch (mpI2CRegs->I2STAT)
    {
        ...
        //slave transmitter
        case slaveReadAckSent:  								//0xA8 similar 0x40
        	puts("Current State: 0xA8");
            if(mTransaction.trxSize > 1) {
                   setAckFlag();  // 1+ bytes: Send ACK to receive a byte and transition to dataAckedByMaster
                   //go to 0xB8
               }
               clearSIFlag();
        	break;
        case dataAckedByMaster:									//0xB8
        	puts("Current State: 0xB8");
            if(mTransaction.trxSize > 1) {
                setAckFlag();  // 1+ bytes: Send ACK to receive a byte and transition to dataAckedByMaster
            }
           //ACK go to 0xC8 ICONSET
            	/*
            	 * if ack -> go to c8
            	 * else -> go to c0
            	 */
                setNackFlag();  //  1 byte : NACK next byte to go to  for 1-byte read.
            	//NACK go to 0xC0

        	break;
/*        case lastDataAckedByMaster:								//0xC8
        	puts("Current State: 0xC8");
        	setAckFlag();
        	break;*/
        case dataNackedByMaster:								//0xC0
        	puts("Current State: 0xC0");
        	setNackFlag();
        	break;


        	//slave Receiver
        case selfSlaveAddressAck:								//0x60
        	puts("Current State: 0x60");
        	setAckFlag();
        	break;

        case generalCallAcked:									//0x70
        	puts("Current State: 0x70");
            if(0 == mTransaction.trxSize) {
                setStop();
            }
            else {
        ...
        case prevDataReceivedAckedbySlave:						//0x80
        	puts("Current State: 0x80");
            if (I2C_READ_MODE(mTransaction.slaveAddr)) {
                setSTARTFlag(); // Send Repeat-start for read-mode
                clearSIFlag();
            }
        	...
        	setAckFlag();
        	break;
        case prevDataReceivedNackedbySlave:							//0x88
        	puts("Current State: 0x88");
            if (I2C_READ_MODE(mTransaction.slaveAddr)) {
                setSTARTFlag(); // Send Repeat-start for read-mode
                clearSIFlag();
            }
        	...
        	setNackFlag();
        	break;
/*        case stopReceieved:										//0xA0
        	setStop();
        	break;*/

        default:
        	puts("Current State: Stop");
            mTransaction.error = mpI2CRegs->I2STAT;
            setStop();
            break;
